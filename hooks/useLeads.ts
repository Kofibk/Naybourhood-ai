...
function mapBuyerRow(b: any): Buyer {
  ...
  return {
    ...
    budget:
      b.budget || b.budget_range || b["budget range"] || b["Budget Range"],
    ...
    timeline:
      b.timeline || b["timeline to purchase"] || b["Timeline to Purchase"],
    ...
    source_campaign:
      b.source_campaign || b["Source Campaign"] || b["source campaign"],
    ...
    payment_method:
      b.payment_method || b["cash or mortgage"] || b["Cash or Mortgage"],
    mortgage_status:
      b.mortgage_status || b["manual update"] || b["Manual Update"],
    ...
    created_at:
      b.date_added || b.created_at || b["date added"] || b["Date Added"],
    ...
    ready_in_28_days:
      b.ready_in_28_days ?? b["Ready in 28 Days"] ?? b.ready_in_28days,
    viewing_intent_confirmed:
      b.viewing_intent_confirmed ?? b["Viewing Intent"] ?? b.viewing_intent,
    ...
    last_wa_message:
      b.last_wa_message || b["Last WA Message"] || b.last_whatsapp_message,
        ...
        fetch("/api/ai/score-buyer", {
          ...
          .then(async (res) => {
            if (res.ok) {
              ...
              queryClient.setQueryData<{ data: Buyer[]; count: number }>(
                ["leads", page, pageSize],
                (old) => {
                  if (!old) return old;
                  return {
