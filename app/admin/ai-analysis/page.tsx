    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <div className="bg-[#111111] border border-white/10 rounded-xl">
          <div className="p-4">
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs text-white/50">Cold Leads</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Pipeline Health */}
        <div className={`bg-[#111111] border rounded-xl ${analysis ? getHealthBg(analysis.pipelineHealth.score) : 'border-white/10'}`}>
          ...
          <div className="px-4 pb-4">
            {isAnalyzing && !analysis ? (
              ...
              <p className="text-sm text-white/50">Click &ldquo;Run Analysis&rdquo; to generate insights</p>
            )}
          </div>
        </div>
            ...
            {isAnalyzing && !analysis ? (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-3 rounded-lg bg-blue-500/10 border border-blue-500/20">
                  <div className="text-xs text-white/50">Expected Viewings</div>
                  <div className="text-2xl font-bold text-blue-500">{analysis.predictions.viewings}</div>
                </div>
                <div className="p-3 rounded-lg bg-green-500/10 border border-green-500/20">
                  <div className="text-xs text-white/50">Expected Reservations</div>
                ...
                <div className="p-3 rounded-lg bg-red-500/10 border border-red-500/20">
                  <div className="text-xs text-white/50">At-Risk Leads</div>
                  <div className="text-2xl font-bold text-red-500">{analysis.predictions.atRiskLeads}</div>
                </div>
              </div>
            ) : (
              <p className="text-sm text-white/50">Click &ldquo;Run Analysis&rdquo; to generate predictions</p>
            )}
          </div>
        </div>
      </div>
          ...
          {isAnalyzing && !analysis ? (
            ...
            <div className="space-y-4">
              {analysis.sourcePerformance.map((source, i) => (
                <div key={i} className="flex flex-col sm:flex-row sm:items-center gap-4 p-3 rounded-lg bg-white/[0.02]">
            ...
            <div className="space-y-3">
              {analysis.bottlenecks.map((bottleneck, i) => (
                <div key={i} className="p-4 rounded-lg bg-warning/10 border border-warning/20">
                  ...
                  <p className="text-sm text-white/50">{bottleneck.recommendation}</p>
                </div>
              ))}
            </div>
          ) : (
